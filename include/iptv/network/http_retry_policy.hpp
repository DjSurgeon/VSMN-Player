...
enum class BackoffStrategy : std::uint8_t {
  /**
   * @brief No delay between retries (immediate retry).
   */
  None,
  /**
   * @brief Wait a fixed amount of time (initial_delay) between each retry.
   */
  Fixed,
  /**
   * @brief Double the wait time after each failure, capped at max_delay.
   */
  Exponential
};
...
struct RetryPolicy {
  ...
  /**
   * @brief Base delay before the first retry (or constant delay for Fixed).
   */
  ...
  /**
   * @brief The backoff calculation strategy to use.
   */
