...
HttpResponse HttpClient::download(const std::string& url, std::chrono::milliseconds timeout) {
    ...
    for (std::uint32_t attempt = 0; attempt <= retries; ++attempt) {
        ...
        metrics.total_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_wall_clock - start_wall_clock);
        ...
        const bool is_transient = (res == CURLE_OPERATION_TIMEDOUT || res == CURLE_COULDNT_CONNECT || res == CURLE_COULDNT_RESOLVE_HOST) ||
